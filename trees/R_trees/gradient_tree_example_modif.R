...
plot_tree = function(tree_file, meta){
  ...
  t = ggtree(tree_rooted, size=1) %<+% info + geom_tiplab(size=4, aes(color=label), fontface = "bold") +
    scale_color_manual(values=info$color) + geom_treescale(y=-1) + theme(legend.position = "none")
...
for (file in trees){
  ...
  g = plot_tree(file,
                meta_path)  +
    ggtitle(strsplit(strsplit(file, '/')[[1]][2], '.nwk')[[1]][1]) + xlim(c(0, 9.5))
  # save figure in png format. svg, pdf are also possible
  ggsave(paste(file, "png", sep="."),height=14, width=19, dpi=1200)
}
