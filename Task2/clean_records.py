...
def check_collection_date(fasta):
    with open(fasta, 'r') as file:
        for line in file:
            for i in range(1900, 2025):
                if ">" in line:
                    no_coll_date.append(line.strip(">\n"))
    with open("records_no_collection_date.txt", 'w') as writer:
        ...
        for fasta_header in list(set(no_coll_date)): # Превращение в set(no_coll_date) и обратно - для отсеивания повторяющихся значений списка
