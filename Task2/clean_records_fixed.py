...
def clean_records(fasta_file):
    seqs_cleaned = []
    for rec_num, seq in enumerate(SeqIO.parse(fasta_file, "fasta")): # ЮА: номер записи называть record никто не запрещает, но это путает, так как обычно так называют объекты SeqRecord
