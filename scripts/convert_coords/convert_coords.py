...
def find_aln_region_by_ref_coords(ref_name, left_bound, right_bound, inp_aln, save_dir):
    global df_aln_coords
    for ref_fasta in SeqIO.parse(f'{save_dir}/{ref_name}_no_gaps.fas', 'fasta'):
        subseq = str(ref_fasta.seq)[int(left_bound) - 1:int(right_bound)].upper()
        for ref in SeqIO.parse(inp_aln, 'fasta'):
            if ref.id == ref_name:
                aln_ref = re.findall(f"[{''.join('-')}]*".join(subseq), str(ref.seq))
                new_left_bound = f"{str(ref.seq).index(aln_ref[0]) + 1}"
                new_right_bound = f"{str(ref.seq).index(aln_ref[0]) + len(aln_ref[0])}"

                print(f"Reference {ref_name}: {left_bound}-{right_bound}, "
                      f"alignment: {new_left_bound}-{new_right_bound}")

                df_aln_coords = df_aln_coords._append({'new_left_bound': new_left_bound,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ic", "--input_coord", type=str,
                        help="Path to csv file with coordinates. Must contain two columns with"
                             "left and right boundaries of the genomic regions you'd like to cut", required=True),
...
df_aln_coords = pd.DataFrame(columns=['new_left_bound', 'new_right_bound'])
