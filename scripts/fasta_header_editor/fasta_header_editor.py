...
def edit_fasta(path_old, path_new):
    # Файл с аннотацией
    ser = pd.read_csv(r"ORF2_RNA_tree_ann.txt",
                      sep="\t",
                      header=None,
                      skiprows=3) \
          .drop(columns=[1, 3, 4]) \
    ...
    ser['serotype'] = np.select(conditions, choices, default=np.array(np.nan, dtype='object'))
...
edit_fasta(r"ORF2_cleared_alignment.fas",
           r"ORF2_cleared_alignment_serotypes.fas")
